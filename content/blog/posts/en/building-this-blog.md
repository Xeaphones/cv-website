...
I added a simple CI/CD pipeline for LWS hosting with GitHub Actions:

- `npm ci` then `npm run build` on each push to `main`
- deploy `dist/` over SSH with `rsync --delete`
- markdown updates are now published automatically after push

This keeps the content workflow close to Astro-like authoring: edit markdown in git, push, and let CI rebuild + deploy.
